...
class TestCase(unittest.TestCase):
    ...
    def test_configure_loopback(self):
        response = requests.put(self.base + "loopback/1", { 'description': 'test loopback', 'ip_address':'10.1.1.1', 'mask':'255.255.255.0'})
        self.assertTrue('ok' in response.text)

    def test_configure_loopback_no_description(self):
        response = requests.put(self.base + "loopback/3", { 'no_description': 'test loopback', 'ip_address':'10.1.1.1', 'mask':'255.255.255.0'})
        self.assertTrue('description for the loopback is required' in response.text)

    def test_configure_loopback_no_ip(self):
        response = requests.put(self.base + "loopback/3", { 'description': 'test loopback', 'no_ip_address':'10.1.1.1', 'mask':'255.255.255.0'})
        self.assertTrue('ip_address for the loopback is required' in response.text)

    def test_configure_loopback_no_mask(self):
        response = requests.put(self.base + "loopback/3", { 'description': 'test loopback', 'ip_address':'10.1.1.1', 'no_mask':'255.255.255.0'})
        self.assertTrue('mask for the loopback is required' in response.text)

    def test_configure_loopback_invalid_ip_mask(self):
        response = requests.put(self.base + "loopback/3", { 'description': 'test loopback', 'ip_address':'1.1.1.1', 'mask':'0.0.0.0'})
